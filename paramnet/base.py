...
_add_methods = ['add_node', 'add_nodes_from', 'add_edge', 'add_edges_from', 'add_weighted_edges_from']


def verify_add(method):
    """ wrapper to check all node/edge additions have required parameters and rollback if not

        Checking is done by casting the attr dicts for any new nodes/edges as the appropriate AttrDict
        subclass in this module, which enforces required keys. Cleanest general solution, since
        (i) nx.add_node/edge* methods accept different input types and (ii) they don't consistently use
...
class ParametrizedMeta(type):
    ...
    def __call__(mcs, *args, **kwargs):
        ...
        if not isinstance(obj, nx.Graph):
            raise TypeError("Parametrize mixin must be used with a subclass of nx.Graph.")
        return obj
